...
app = Flask(__name__)
CORS(app)  # Enable CORS for all routes - VERY IMPORTANT

@app.route('/api/price-history/<symbol>')
def get_price_history(symbol):
    try:
        period = request.args.get('period', '3mo')  # Default to 3 months if not specified
        interval = request.args.get('interval', '1d')  # Default to daily if not specified

        ticker = yf.Ticker(symbol)

        # Set appropriate period and interval combinations
        period_settings = {
            '1d': {'period': '1d', 'interval': '5m'},     # 5-minute intervals for 1 day
            '1w': {'period': '1w', 'interval': '1h'},    # 15-minute intervals for 1 week
            '1mo': {'period': '1mo', 'interval': '1d'},    # 1-hour intervals for 1 month
            '3mo': {'period': '3mo', 'interval': '1d'},    # Daily intervals for 3 months
            '6mo': {'period': '6mo', 'interval': '1d'},    # Daily intervals for 6 months
            '1y': {'period': '1y', 'interval': '1d'},      # Daily intervals for 1 year
            'max': {'period': 'max', 'interval': '1wk'}     # Weekly for max period - adjust interval if needed for very long ranges
        }
        ...
        if hist.empty:  # Check if hist DataFrame is empty (no data found)
            return jsonify({
                "prices": [],
                "meta": {
                    "lastPrice": None,
        ...
        print(f"Error fetching price history for {symbol} with period={period} and interval={interval}: {e}") # Enhanced error logging
