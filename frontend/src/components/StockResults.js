...
const StockResults = () => {
    ...
    useEffect(() => {
        const fetchStockResults = async () => {
            ...
            setError(null);
            try {
                const companyDetailsResponse = await axios.get(`http://127.0.0.1:5000/api/company-details/${symbol}.NS`);
                const companyInfo = companyDetailsResponse.data;
                setCompanyDetails(companyInfo);

                const stockDataResponse = await axios.get(`/api/stocks/${symbol.toUpperCase()}`);
