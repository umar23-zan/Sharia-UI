...
useEffect(() => {
  const fetchUserData = async () => {
      if (email) {
          ...
          setProfilePreview(userData.profilePicture ? `${userData.profilePicture}` : UserRound);
      }
  };
  fetchUserData();
}, [email]);
...
const handleSubmit = async (e) => {
  ...
  if (Object.keys(newErrors).length === 0) {
      ...
      setProfilePreview(updatedUserData.profilePicture ? `${updatedUserData.profilePicture}` : UserRound);
